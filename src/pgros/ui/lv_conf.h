/**
 * @file lv_conf.h
 * PgrOS LVGL 9.3.0 configuration -- LilyGo T-LoRa Pager (ESP32-S3).
 *
 * Derived verbatim from lvgl 9.3.0's lv_conf_template.h; every deviation from
 * the template is marked with a "PgrOS:" comment so a version bump can be done
 * by re-templating and re-applying just those hunks.
 *
 * Reached via -D LV_CONF_INCLUDE_SIMPLE, so this file must sit on an include
 * path (see pgros.ini). Panel is 480x222 landscape, RGB565, partial render.
 */
 ...
 * - LV_STDLIB_CUSTOM:      Implement the functions externally
 */
#define LV_USE_STDLIB_STRING    LV_STDLIB_CLIB   /* PgrOS: newlib memcpy/strlen are already linked and faster */
 ...
 * - LV_STDLIB_CUSTOM:      Implement the functions externally
 */
#define LV_USE_STDLIB_SPRINTF   LV_STDLIB_CLIB   /* PgrOS: newlib vsnprintf, avoids a second formatter in flash */
...
#define LV_STDARG_INCLUDE       <stdarg.h>
    ...
    /* PgrOS: LVGL keeps its builtin TLSF allocator, but the pool it manages is
     * taken from PSRAM rather than from a static array in internal RAM.
     *
     * Why BUILTIN and not LV_STDLIB_CLIB: on ESP32-S3 Arduino, malloc() serves
     * internal DRAM, which is the scarcest resource on this board -- WiFi, the
     * BLE stack, FreeRTOS stacks and DMA descriptors all have to live there.
     * Routing LVGL through CLIB would put every style, object and layer buffer
     * into internal RAM. A single TLSF pool carved out of the 8MB PSRAM is both
     * bigger and better behaved: one allocation at lv_init(), no fragmentation
     * of the internal heap, and LVGL's own allocation accounting still works.
     *
     * Note that this pool is for LVGL bookkeeping only. The display draw
     * buffers are NOT allocated from here -- Display::beginLvgl() allocates
     * those itself with heap_caps_aligned_alloc(32, ...) because they are DMA
     * sources and need 32-byte alignment that TLSF does not guarantee. */
    #define LV_MEM_SIZE (512 * 1024U)         /**< [bytes] -- PgrOS: PSRAM-backed */
    ...
    #if LV_MEM_ADR == 0
        /* PgrOS: pull the pool from PSRAM. lv_mem_core_builtin.c is C, so this
         * has to be a C header -- esp_heap_caps.h is, Arduino.h is not. */
...
/* PgrOS: 32 == the ESP32-S3 cache line, and the alignment GDMA wants when the
 * source is PSRAM. Draw buffers we allocate ourselves already honour this; this
 * makes LVGL's internal draw-buf allocations (layers, snapshots) match.
 * LV_DRAW_BUF_STRIDE_ALIGN stays at 1 on purpose: with stride == width * 2 the
 * partial buffer handed to flush_cb is contiguous, which is what lets us hand
 * it straight to pushImageDMA() with no per-row fixup. */
...
#define LV_USE_DRAW_SW 1
     ...
     */
    /* PgrOS: the panel is RGB565 and PgrOS ships no bitmap assets, so only the
     * formats LVGL actually reaches for are compiled in. RGB888/XRGB8888/
     * ARGB8888 stay because gradients and opacity layers use them internally;
     * A8 stays because it is the font glyph mask format. RGB565_SWAPPED is off
     * because we render native-endian and byte-swap once in the flush cb. */
...
#define LV_ASSERT_HANDLER_INCLUDE <stdlib.h>   /* PgrOS */
/* PgrOS: the template spins forever, which on a headless pager looks exactly
 * like a hang and leaves no evidence. abort() takes the ESP32 panic path:
 * backtrace over serial, core dump, then reset. */
...
#define LV_OBJ_STYLE_CACHE      1   /* PgrOS: 8 bytes/obj for a measurable redraw win; PSRAM can afford it */
...
#define LV_FONT_MONTSERRAT_12 1   /* PgrOS: status bar, timestamps */
#define LV_FONT_MONTSERRAT_14 1
#define LV_FONT_MONTSERRAT_16 0
#define LV_FONT_MONTSERRAT_18 0
#define LV_FONT_MONTSERRAT_20 1   /* PgrOS: clock, headings */
 ...
 * */
/* PgrOS: the widget set is trimmed to what the pager UI actually builds. Kept:
 * arc, bar, button, buttonmatrix, checkbox, dropdown, image, label, line, list,
 * msgbox, roller, slider, spinner, switch, textarea. Everything switched off
 * below is off purely for flash; flip one back on if an app needs it.
 * lv_keyboard in particular is off because the pager has a physical keyboard
 * (pgros.ini already drops Meshtastic's VirtualKeyboard.cpp). */
#define LV_WIDGETS_HAS_DEFAULT_VALUE  1

#define LV_USE_ANIMIMG    0   /* PgrOS: unused */
...
#define LV_USE_CALENDAR   0   /* PgrOS: unused */
#if LV_USE_CALENDAR
...
#define LV_USE_CANVAS     0   /* PgrOS: unused */

#define LV_USE_CHART      0   /* PgrOS: unused */
...
#define LV_USE_IMAGEBUTTON     0   /* PgrOS: unused */

#define LV_USE_KEYBOARD   0   /* PgrOS: unused */

#define LV_USE_LABEL      1
#if LV_USE_LABEL
    #define LV_LABEL_TEXT_SELECTION 0   /* PgrOS: no touch panel, nothing can drag-select */
    #define LV_LABEL_LONG_TXT_HINT 1    /**< Store some extra info in labels to speed up drawing of very long text */
    #define LV_LABEL_WAIT_CHAR_COUNT 3  /**< The count of wait chart */
#endif

#define LV_USE_LED        0   /* PgrOS: unused */
...
#define LV_USE_MENU       0   /* PgrOS: unused */

#define LV_USE_MSGBOX     1

#define LV_USE_ROLLER     1   /**< Requires: lv_label */

#define LV_USE_SCALE      0   /* PgrOS: unused */

#define LV_USE_SLIDER     1   /**< Requires: lv_bar */

#define LV_USE_SPAN       0   /* PgrOS: unused */
#if LV_USE_SPAN
    /** A line of text can contain this maximum number of span descriptors. */
    #define LV_SPAN_SNIPPET_STACK_SIZE 64
#endif

#define LV_USE_SPINBOX    0   /* PgrOS: unused */

#define LV_USE_SPINNER    1

#define LV_USE_SWITCH     1

#define LV_USE_TABLE      0   /* PgrOS: unused */

#define LV_USE_TABVIEW    0   /* PgrOS: unused */
...
#define LV_USE_TILEVIEW   0   /* PgrOS: unused */

#define LV_USE_WIN        0   /* PgrOS: unused */
...
#define LV_USE_THEME_DEFAULT 1
    ...
    #define LV_THEME_DEFAULT_DARK 1   /* PgrOS: dark is the default; see ui/Theme.h */
...
#define LV_USE_THEME_SIMPLE 0   /* PgrOS: unused */

/** A theme designed for monochrome displays */
#define LV_USE_THEME_MONO 0   /* PgrOS: unused */
...
#define LV_BUILD_EXAMPLES 0   /* PgrOS */

/** Build the demos */
#define LV_BUILD_DEMOS 0   /* PgrOS */
